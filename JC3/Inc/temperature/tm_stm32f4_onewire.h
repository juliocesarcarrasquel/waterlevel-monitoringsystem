...
#define ONEWIRE_DELAY(x)				TIM6_MicrosecondsDelay(x);
...
#define ONEWIRE_INPUT(structure) 		do{ \
	uint8_t i; \
	for (i = 0x00; i < 0x10; i++) { \
		if ((structure)->GPIO_Pin & (1 << i)) { \
			(structure)->GPIOx->MODER &= ~(0x03 << (2 * i)); \
		} \
	} \
    } while( 0 )
...
#define ONEWIRE_OUTPUT(structure) 		do{ \
	...
	for (i = 0x00; i < 0x10; i++) { \
		if ((structure)->GPIO_Pin & (1 << i)) { \
			(structure)->GPIOx->MODER = ((structure)->GPIOx->MODER & ~(0x03 << (2 * i))) | (0x01 << (2 * i)); \
		} \
	} \
    } while( 0 )
